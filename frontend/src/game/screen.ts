...
and perhaps, convert cells number, but generally the single array approach is faster
*/
const weak_obstacles = new Map<string, HTMLDivElement>() // keep weak obstacles, to manipulate them later
const power_ups = new Map<string, HTMLDivElement>() // keep powerups, to manipulate them later
const bombs = new Map<string, HTMLDivElement>() // keep bombs, to manipulate them later
const players = new Map<string, HTMLDivElement>() // keep players, to manipulate them later
const explosions = new Map<string, HTMLDivElement>() // keep explosions, to manipulate them later
...
export async function prebuild_game_field(): Promise<void> {
  ...
  for (let i = 0; i < 8; i++) {
    const div = document.createElement("div");
    div.classList.add("cell"); // for static objects
    div.classList.add("weak_obstacle");
    div.style.left = `${dx[i] * cspx}px`;
    div.style.top = `${dy[i] * cspx}px`;
    game_weak_obstacles.appendChild(div);
    weak_obstacles.set(`${dx[i]}${dy[i]}`, div) // always less then 10, so no need space between x and y
  }
  ...
  for (let i = 0; i < 8; i++) {
    ...
    div.classList.add("power_up1"); //todo: remove this later, server will decide where and what powerup will be placed
    div.style.left = `${dx[i] * cspx}px`;
    div.style.top = `${dy[i] * cspx}px`;
    game_power_ups.appendChild(div);
    power_ups.set(`${dx[i]}${dy[i]}`, div) // always less then 10, so no need space between x and y
  }
  ...
  for (let i = 0; i < 40; i++) {
    const div = document.createElement("div");
    div.classList.add("absolute"); // for animated objects, to do not lock size
    div.classList.add("player1_bomb"); //todo: remove this later, server will decide where and what powerup will be placed
    div.style.left = `${dx[i] * cspx}px`;
    div.style.top = `${dy[i] * cspx}px`;
    game_bombs.appendChild(div);
    bombs.set(`${dx[i]}${dy[i]}`, div) // always less then 10, so no need space between x and y
  }
  ...
  for (let i = 0; i < 40; i++) {
    const div = document.createElement("div");
    div.classList.add("absolute"); // for animated objects, to do not lock size
    div.classList.add("explosion"); //todo: remove this later, server will decide where and what powerup will be placed
    div.style.left = `${dx[i] * cspx}px`;
    div.style.top = `${dy[i] * cspx}px`;
    game_explosions.appendChild(div);
    explosions.set(`${dx[i]}${dy[i]}`, div) // always less then 10, so no need space between x and y
  }
  ...
  for (let i = 0; i < 4; i++) {
    const div = document.createElement("div");
    div.classList.add("absolute"); // for animated objects, to do not lock size
    div.classList.add(`player${i + 1}_move`); //todo: remove this later, server will decide where and what powerup will be placed
    div.style.left = `${dx[i] * cspx}px`;
    div.style.top = `${dy[i] * cspx}px`;
    game_players.appendChild(div);
    players.set(`${dx[i]}${dy[i]}`, div) // always less then 10, so no need space between x and y
  }
}