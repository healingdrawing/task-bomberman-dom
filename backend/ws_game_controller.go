...
func ws_up_handler(number int, control string) {
	...
	if !player.moving {
		target_position_cell := fmt.Sprint(player.X) + fmt.Sprint(player.Y-1)
		...
		player.moving_start_time_stamp = time.Now().UnixNano()
		...
		player.Target_y == player.Y-1 &&
		unix_ts-player.moving_start_time_stamp > player.one_cell_move_duration/10*8-2 {
		// check if 80% of timer is done, and player still wants to move up
		// if yes, then move/aim player up one cell more if possible
		target_position_cell := fmt.Sprint(player.X) + fmt.Sprint(player.Y-2)
		if _, ok := game.free_cells[target_position_cell]; !ok {
			return
		}
